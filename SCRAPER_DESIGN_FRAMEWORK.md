...
GET /report – Latest run summary + case list. By default this reads the legacy JSON logs; when ``BAILIIKC_USE_DB_REPORTING=1`` it instead pulls rows from SQLite via ``db_reporting`` while keeping the row shape identical (``actions_token``, ``title``, ``subject``, ``court``, ``category``, ``judgment_date``, ``sort_judgment_date``, ``cause_number``, ``downloaded_at``, ``saved_path``, ``filename``, ``size_kb``).

GET /api/runs/latest, /api/downloaded-cases – JSON-backed endpoints used by the existing UI/report. ``/api/downloaded-cases`` switches to the DB path when ``BAILIIKC_USE_DB_REPORTING=1``.
